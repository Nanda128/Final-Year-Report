...
This central problem is addressed through three design priorities:

\begin{itemize}
    \item Designing a simulation and C3I (Command, Control, Communications, and Intelligence) framework that captures key MSAR constraints and can evolve toward a full Digital Twin system.
    \item Designing an interface that exposes the structure of the search problem and high-level swarm autonomy decisions without overloading the operator.
    \item Defining an evaluation approach that combines operational performance metrics with workload-oriented assessment methods.
\end{itemize}

These priorities are rationalised as formal research questions and hypotheses in Section~\ref{subsec:objectives-and-hypotheses}.

\subsection{Contributions}\label{subsec:contributions}

This project seeks to contribute, through the design and implementation of BEACON, a multi-layer Digital Twin framework for MSAR drone swarms.
...
The alert hierarchy is relevant to BEACON because MSAR supervision is time-critical, and severity-based alerting helps reduce the effort spent triaging issues while controlling drones.
...
BEACON's simulation-first, and modular architecture allows the same mission scenario to be replayed under consistent baseline conditions, while varying different factors at a time, such as drone, communication fidelity, or environmental difficulty.
The architecture's extensibility also allows for comparisons between different interface designs, swarm controls, and more, while keeping the same underlying scenario and mechanics.
These features allow BEACON to enable systematic, controlled testing of human-swarm MSAR interfaces before being deployed in costly real-world scenarios.

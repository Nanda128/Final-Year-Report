...
BEACON's simulation interface utilizes a single, stable frame.
...
Below it, is the Alerts panel, providing a searchable list of active and past alerts, each alert accompanied by a visual and audio component, categorized into four different alert categories as per the standards followed by most Aviation Crew Alerting Systems (\cite{RN3}).
...
At the center of the map is a ``Drone Hub'' from which users can choose to spawn drones at, if they don't opt to spawn the drones from the corners fo the map.

The color palette of the interface is designed to meet WCAG 2.2 AA standards for contrast, with a focus on blues and teals to reflect the maritime domain, with bright accent colors used for alerts and important information to draw attention without overwhelming the user.
The colors of the alerts follow standards for aviation alerting systems, with red for critical alerts, amber for warnings, blue for advisories, and green for system messages.
...
These are defined using ``Theme Tokens'', which define the colors of each font, background, and accent color across the interface depending on the selected theme, to ensure a consistent visual identity across the interface.
By default, the system starts in the preferred color scheme of the user's operating system, but users can choose to override this in the top right.
...
They were also easier to capture within the simulation, and similarly easier to analyze across different runs of the simulation, compared to more complex metrics like weighted situational awareness scores, which would be difficult to capture without real-world testing and human subject research.
